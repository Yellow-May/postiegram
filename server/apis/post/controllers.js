...
module.exports.GET_SINGLE_POST = async (req, res) => {
	...
	const { _id, caption, media, likes, createdAt } = await PostModel.findOne({ creator_id: user._id, _id: post_id })
		.select('_id caption media likes createdAt')
		.sort({ createdAt: 'desc' })
		.exec();
...
module.exports.GET_FOLLOWING_POSTS = async (req, res) => {
	...
	const posts = await Promise.all(
		raw_posts.map(async post => {
			const { _id, creator_id, caption, media, likes, createdAt } = post;
...
module.exports.TOGGLE_LIKE = async (req, res) => {
	...
	res.status(StatusCodes.OK).json({ message: like_id ? 'Unliked post' : 'Liked post' });
};
